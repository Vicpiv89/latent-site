...
(function () {
  ...
  /* Section-level reveals are adopted here rather than marked up per page
     (2026-09-08, Vic: "entrance animations"): every deck component below the
     fold rises on approach. The first fold is excluded on purpose - it has its
     own load-in keyframes in styles.css section 19 and must never start hidden. */
  var AUTO_REVEAL = ".sec > .wrap > .head, .sec > .wrap > .foot, .sec > .wrap > p, " +
    ".deck-card, .plan, .fr-card, .enq-card, .stat-row > .stat, .funnel-shot, " +
    ".sec .tbl-wrap, .livecard, .board-grid > *, .section > .wrap > .panel, " +
    ".fr-steps > *, .plans-note, .quote, .cmp .tbl-wrap, " +
    "main > section:not(:first-of-type) > .wrap > *";
  var AUTO_GROUP = ".grid-2, .grid-3, .grid-4, .stat-row, .plans-grid, .fr-gallery, .board-grid, .fr-steps";
  var FOLD = ".hero-ink, .fr-hero, .hero-brand, .phero, .led-hero, .m-hero, .site-nav, .site-footer, main > section:first-of-type";

  function adoptReveals(scope) {
    var i, list;
    list = scope.querySelectorAll(AUTO_GROUP);
    for (i = 0; i < list.length; i++) {
      if (!list[i].closest(FOLD) && !list[i].hasAttribute("data-reveal-group")) list[i].setAttribute("data-reveal-group", "");
    }
    list = scope.querySelectorAll(AUTO_REVEAL);
    for (i = 0; i < list.length; i++) {
      var el = list[i];
      if (el.hasAttribute("data-reveal") || el.closest(FOLD) || el.closest("[data-motion-off]")) continue;
      if (el.querySelector("[data-reveal]")) continue;     // never nest reveals
